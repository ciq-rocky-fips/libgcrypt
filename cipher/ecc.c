...
ecc_generate (const gcry_sexp_t genparms, gcry_sexp_t *r_skey)
{
  ...
  rc = _gcry_mpi_ec_internal_new (&ec, &flags, "ecgen curve", genparms, NULL, fips_strict);
  if (rc)
    goto leave;
...
ecc_check_secret_key (gcry_sexp_t keyparms)
{
  ...
  int flags = 0;
  mpi_ec_t ec = NULL;
  int fips_strict = 1; /* CHECKME_FIPS */

  /*
   * Extract the key.
   */
  rc = _gcry_mpi_ec_internal_new (&ec, &flags, "ecc_testkey", keyparms, NULL, fips_strict);
  if (rc)
    goto leave;
...
static gcry_err_code_t
ecc_sign (gcry_sexp_t *r_sig, gcry_sexp_t s_data, gcry_sexp_t keyparms, int fips_strict)
{
  gcry_err_code_t rc;
   ...
   * Extract the key.
   */
  rc = _gcry_mpi_ec_internal_new (&ec, &flags, "ecc_sign", keyparms, NULL, fips_strict);
  if (rc)
    goto leave;
...
static gcry_err_code_t
ecc_verify (gcry_sexp_t s_sig, gcry_sexp_t s_data, gcry_sexp_t s_keyparms, int fips_strict)
{
  gcry_err_code_t rc;
  struct pk_encoding_ctx ctx;
  gcry_sexp_t l1 = NULL;
...
ecc_encrypt_raw (gcry_sexp_t *r_ciph, gcry_sexp_t s_data, gcry_sexp_t keyparms)
{
   ...
   * Extract the key.
   */
  rc = _gcry_mpi_ec_internal_new (&ec, &flags, "ecc_encrypt", keyparms, NULL, fips_strict);
  if (rc)
    goto leave;
...
ecc_decrypt_raw (gcry_sexp_t *r_plain, gcry_sexp_t s_data, gcry_sexp_t keyparms)
{
   ...
   * Extract the key.
   */
  rc = _gcry_mpi_ec_internal_new (&ec, &flags, "ecc_decrypt", keyparms, NULL, fips_strict);
  if (rc)
    goto leave;
...
#define N_COMPONENTS 6
  ...
  int fips_strict = 0; /* CHECKME_FIPS */
