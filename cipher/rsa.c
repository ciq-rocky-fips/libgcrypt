...
test_keys_fips (gcry_sexp_t skey)
{
  ...
  /* Use the RSA public function to verify this signature.  */
  ec = _gcry_pk_verify_md (sig, data_tmpl, hd, skey, NULL, fips_strict);
  if (ec)
    goto leave;
...
static gcry_err_code_t
rsa_sign (gcry_sexp_t *r_sig, gcry_sexp_t s_data, gcry_sexp_t keyparms, int fips_strict)
{
  gpg_err_code_t rc;
  struct pk_encoding_ctx ctx;
  gcry_mpi_t data = NULL;
...
static gcry_err_code_t
rsa_verify (gcry_sexp_t s_sig, gcry_sexp_t s_data, gcry_sexp_t keyparms, int fips_strict)
{
  gcry_err_code_t rc;
  struct pk_encoding_ctx ctx;
  gcry_sexp_t l1 = NULL;
