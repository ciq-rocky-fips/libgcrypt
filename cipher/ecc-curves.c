      ...
      if (l1)
        {
          ...
          errc = _gcry_pk_util_parse_flaglist (l1, &flags, NULL, fips_strict);
          sexp_release (l1);
          l1 = NULL;
          if (errc)
            goto leave;
...
_gcry_mpi_ec_internal_new (mpi_ec_t *r_ec, int *r_flags, const char *name_op,
                           gcry_sexp_t keyparam, const char *curvename, int fips_strict)
{
  gpg_err_code_t errc;
  elliptic_curve_t E;
  mpi_ec_t ec;
  ...
  *r_ec = NULL;
  ...
  memset (&E, 0, sizeof E);
  errc = mpi_ec_get_elliptic_curve (&E, r_flags, keyparam, curvename, fips_strict);
  if (errc)
    goto leave;
                  ...
                  gcry_sexp_t keyparam, const char *curvename)
{
  ...
  *r_ctx = NULL;
  ...
  memset (&E, 0, sizeof E);
  errc = mpi_ec_get_elliptic_curve (&E, &flags, keyparam, curvename, fips_strict);
  if (errc)
    goto leave;
